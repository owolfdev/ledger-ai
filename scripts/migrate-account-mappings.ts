...
/**
 * Migration script to populate database tables with existing account mapping patterns
 * Run with: npx tsx scripts/migrate-account-mappings.ts
 */
...
// Extract patterns from the existing account-map.ts file
const DESC_CATEGORY_RULES = [
  ...
  // Utilities & Housing
  { pattern: /(rent|rental)/i, category: "Housing:Rent" },
  {
    pattern: /(electricity|electric|power)/i,
    category: "Utilities:Electricity",
  },
  { pattern: /(water|water\s*bill)/i, category: "Utilities:Water" },
  {
    pattern: /(internet\s*home|wifi|broadband)/i,
    category: "Utilities:Internet",
  },
  { pattern: /(phone\s*bill|mobile\s*bill)/i, category: "Utilities:Phone" },
  {
    pattern: /(condo\s*fee|management\s*fee|maintenance\s*fee)/i,
    category: "Housing:Fees",
  },
  { pattern: /(cleaning\s*supplies|appliances)/i, category: "Home:Supplies" },
  ...
  { pattern: /(itunes|app\s*store)/i, category: "Subscription:Apple" },
  {
  ...
  { pattern: /(laptop|notebook|ultrabook)/i, category: "Electronics:Laptops" },
  {
  ...
  { pattern: /(lazada|online\s*shopping)/i, category: "Shopping:Online" },
  {
    pattern: /(jewelry|accessories|bag|purse)/i,
    category: "Shopping:Accessories",
  },
  { pattern: /(furniture|home\s*decor)/i, category: "Shopping:Home" },
  ...
  { pattern: /(japanese|sushi|ramen|sashimi)/i, category: "Food:Japanese" },
  { pattern: /(korean|kimchi|bulgogi)/i, category: "Food:Korean" },
  { pattern: /(chinese|dim\s*sum|fried\s*rice)/i, category: "Food:Chinese" },
  { pattern: /(indian|curry|biryani)/i, category: "Food:Indian" },
  { pattern: /(western|international)/i, category: "Food:Western" },
  ...
  { pattern: /(coconut\s*water)/i, category: "Food:Beverages:CoconutWater" },
  {
  ...
  // Healthcare
  { pattern: /(doctor|clinic|hospital|medical)/i, category: "Health:Medical" },
  { pattern: /(dentist|dental)/i, category: "Health:Dental" },
  {
    pattern: /(pharmacy|medicine|vitamins|supplements)/i,
    category: "Health:Pharmacy",
  },
  { pattern: /(massage|spa|salon|haircut|nails)/i, category: "Personal:Care" },
  {
    pattern: /(shampoo|skincare|cosmetics|makeup)/i,
    category: "Personal:Toiletries",
  },
  { pattern: /(gym|fitness|personal\s*training)/i, category: "Health:Fitness" },
  {
  ...
  // Food sub-categories
  { pattern: /(butter|margarine|ghee)/i, category: "Food:Dairy:Butter" },
  {
  ...
  { pattern: /(dairy|dairy\s*product)/i, category: "Food:Dairy" },
  ...
  { pattern: /(pork|bacon|ham|pork\s*chop)/i, category: "Food:Meat:Pork" },
  {
    pattern: /(fish|salmon|tuna|shrimp|prawn|seafood)/i,
    category: "Food:Meat:Seafood",
  },
  { pattern: /(meat|protein)/i, category: "Food:Meat" },
  ...
  { pattern: /(oat|oatmeal|porridge)/i, category: "Food:Grains:Oats" },
  { pattern: /(cereal|breakfast\s*cereal)/i, category: "Food:Grains:Cereal" },
  { pattern: /(grain|grain\s*product)/i, category: "Food:Grains" },
  ...
  { pattern: /(uniform|school\s*uniform)/i, category: "Education:Uniforms" },
  {
  ...
  // Pantry staples
  { pattern: /(jam|jelly|preserve|marmalade)/i, category: "Food:Pantry:Jam" },
  {
  ...
  { pattern: /(pantry|pantry\s*staple)/i, category: "Food:Pantry" },
  ...
  { pattern: /(veg|vegetable|vegetables)/i, category: "Food:Vegetables" },

  // Snacks and Sweets
  { pattern: /(ice\s*cream|chocolate|candy)/i, category: "Food:Snacks:Sweet" },
  {
    pattern: /(chips|crackers|cookies|biscuit)/i,
    category: "Food:Snacks:Savory",
  },
  { pattern: /(seaweed|nuts|dried\s*fruit)/i, category: "Food:Snacks:Healthy" },
  {
  ...
  { pattern: /(grape|grapes|grape\s*bunch)/i, category: "Food:Fruit:Grapes" },
  { pattern: /(mango|mangoes|mango\s*fruit)/i, category: "Food:Fruit:Mangoes" },
  {
    pattern: /(berry|berries|strawberry|blueberry|raspberry)/i,
    category: "Food:Fruit:Berries",
  },
  { pattern: /(lemon|lemons|lime|limes)/i, category: "Food:Fruit:Citrus" },
  { pattern: /(fruit|fruits)/i, category: "Food:Fruit" },
  ...
  { pattern: /(groceries|grocery)/i, category: "Food:Groceries" },
  {
  ...
  { pattern: /(bus|public\s*bus)/i, category: "Transport:Bus" },
  {
  ...
  { pattern: /(flight|airline|plane)/i, category: "Transport:Air" },
  { pattern: /(hotel|accommodation)/i, category: "Transport:Accommodation" },
  { pattern: /(gas|fuel|petrol|diesel|gasoline)/i, category: "Transport:Fuel" },
  { pattern: /(parking|toll|tollway|easypass)/i, category: "Transport:Fees" },
  {
...
async function migrateAccountPatterns() {
  console.log("🔄 Migrating account patterns...");
  ...
  await supabase.from("account_patterns").delete().neq("id", 0);

  const patterns: InsertAccountPattern[] = DESC_CATEGORY_RULES.map(
    (rule, index) => ({
      pattern_type: "regex",
      pattern: rule.pattern.source,
      account_path: `Expenses:${rule.category}`,
      account_type: "expense",
      business_context: null, // Global patterns
      priority: DESC_CATEGORY_RULES.length - index, // Higher priority for earlier patterns
      is_active: true,
    })
  );
  ...
  if (error) {
    console.error("❌ Error migrating account patterns:", error);
    return;
  }

  console.log(`✅ Migrated ${data?.length || 0} account patterns`);
}

async function migrateVendorMappings() {
  console.log("🔄 Migrating vendor mappings...");
  ...
  // Clear existing vendor mappings
  await supabase.from("vendor_mappings").delete().neq("id", 0);

  const vendorMappings: InsertVendorMapping[] = [
    // Exact matches
    ...VENDOR_EXACT.map((vendor) => ({
      vendor_name: vendor.name,
      vendor_pattern: null,
      account_path: `Expenses:${vendor.category}`,
    ...
    // Pattern matches
    ...VENDOR_CONTAINS.map((vendor) => ({
      vendor_name: vendor.pattern.source,
      vendor_pattern: vendor.pattern.source,
      account_path: `Expenses:${vendor.category}`,
      account_type: "expense" as const,
      business_context: null,
      is_active: true,
  ...
  if (error) {
    console.error("❌ Error migrating vendor mappings:", error);
    return;
  }

  console.log(`✅ Migrated ${data?.length || 0} vendor mappings`);
}

async function migrateBusinessContexts() {
  console.log("🔄 Migrating business contexts...");
  ...
  // Clear existing business contexts
  await supabase.from("business_contexts").delete().neq("id", 0);

  const businessContexts = [
    { business_name: "Personal", default_account_type: "expense" as const },
    { business_name: "MyBrick", default_account_type: "expense" as const },
    { business_name: "Channel60", default_account_type: "expense" as const },
  ];
  ...
  if (error) {
    console.error("❌ Error migrating business contexts:", error);
    return;
  }

  console.log(`✅ Migrated ${data?.length || 0} business contexts`);
}

async function main() {
  console.log("🚀 Starting account mapping migration...");

  try {
    await migrateBusinessContexts();
    await migrateAccountPatterns();
    await migrateVendorMappings();
